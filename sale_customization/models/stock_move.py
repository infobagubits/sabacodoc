...
class StockMove(models.Model):
    ...
    # Campo correlato per visualizzare o meno le colonne di unità e quantità secondaria
    display_secondary = fields.Boolean(
        string="Visualizza Colonne Secondarie",
    ...
    def _compute_exceeds_secondary_qty(self):
        # Calcola se la quantità secondaria eccede lo stock disponibile per movimenti in uscita
        for move in self:
    ...
    def _compute_virtual_secondary_qty(self):
        # In questo esempio, la quantità virtuale è uguale alla quantità inserita
        for rec in self:
    ...
    def create(self, vals):
        """
        Dopo che Odoo raggruppa la move, sommiamo anche il x_secondary_qty manualmente.
        """
        ...
        if picking_id and product_id and secondary_to_add:
            ...
            # Garantire che non sia la move appena creata
